                 ...
                 ValentSmsConversationRow *row)
{
  ...
  conversation = gtk_widget_get_ancestor (GTK_WIDGET (row),
                                          VALENT_TYPE_SMS_CONVERSATION);
  self = VALENT_SMS_CONVERSATION (conversation);
                                        ...
                                        int                    position)
{
  ...
  /* If the message has a sender, lookup the contact */
  ...
  else if (valent_message_get_box (message) == VALENT_MESSAGE_BOX_INBOX)
...
valent_sms_conversation_populate (gpointer data)
{
  ...
  if ((upper = gtk_adjustment_get_upper (self->vadjustment)) == 0)
    return G_SOURCE_CONTINUE;

  self->offset = upper - gtk_adjustment_get_value (self->vadjustment);
...
valent_sms_conversation_queue_populate (ValentSmsConversation *self)
{
  ...
  self->populate_id = g_idle_add_full (G_PRIORITY_DEFAULT,
                         ...
                         ValentSmsConversation *self)
{
  ...
  /* If the position is inbetween our pointers we have to handle them */
...
valent_sms_conversation_send_message (ValentSmsConversation *self)
{
  ...
  if (sent)
    ...
    g_warning ("Failed to send message: %s", text);
