...
valent_sms_store_open (ValentSmsStore  *self)
{
  ...
  valent_task_queue_run_check (self->queue, task, valent_sms_store_open_task);
}
                        ...
                        GError         **error)
{
  ...
  valent_task_queue_run_close (self->queue, task, valent_sms_store_close_task);
}
...
valent_sms_store_dispose (GObject *object)
{
  ...
  if (self->queue)
    {
      valent_sms_store_close (self, NULL);
      g_clear_pointer (&self->queue, valent_task_queue_unref);
    }
...
{
  self->queue = valent_task_queue_new ();
}
                              ...
                              gpointer             user_data)
{
  ...
  g_task_set_task_data (task,
                        g_steal_pointer (&messages),
                        (GDestroyNotify)g_ptr_array_unref);
  valent_task_queue_run (store->queue, task, add_messages_task);
                                 ...
                                 gpointer             user_data)
{
  ...
  *task_data = message_id;
  ...
  valent_task_queue_run (store->queue, task, remove_message_task);
                                ...
                                gpointer             user_data)
{
  ...
  *task_data = thread_id;
  ...
  valent_task_queue_run (store->queue, task, remove_thread_task);
                             ...
                             gpointer             user_data)
{
  ...
  valent_task_queue_run (store->queue, task, find_task);
                              ...
                              gpointer             user_data)
{
  ...
  *task_data = message_id;
  ...
  valent_task_queue_run (store->queue, task, get_message_task);
...
valent_sms_store_get_summary (ValentSmsStore *store)
{
  ...
  valent_task_queue_run (store->queue, task, get_summary_task);
                                  ...
                                  gint64          thread_id)
{
  ...
  valent_task_queue_run_sync (store->queue, task, get_thread_date_task);
                                   ...
                                   gpointer             user_data)
{
  ...
  *task_data = thread_id;
  ...
  g_task_set_task_data (task, task_data, g_free);
  valent_task_queue_run (store->queue, task, get_thread_items_task);
}
