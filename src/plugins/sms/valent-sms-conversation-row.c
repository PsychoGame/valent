...
linkify_text (const char *text)
{
  ...
  if G_UNLIKELY (url_regex == NULL)
    url_regex = g_regex_new (URL_PATTERN, URL_FLAGS, 0, &error);

  if G_LIKELY (url_regex != NULL)
    {
      g_autofree char *escaped = NULL;

      escaped = g_markup_escape_text(text, -1);
      ret = g_regex_replace (url_regex, escaped, -1, 0, URL_REPLACE, 0, &error);
    }
